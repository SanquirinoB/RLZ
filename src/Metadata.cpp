...
/*
Metadata::Metadata(unsigned char _save_mode, vector< pair<unsigned long long, unsigned long long> > *_lowcase_runs, unsigned long long _nl_total, unsigned long long _nl_norm, vector< pair<unsigned long long, unsigned long long> > *_nl_ex){
	save_mode = _save_mode;
	lowcase_runs = _lowcase_runs;
	cout<<"Metadata - lowcase_runs: "<<(lowcase_runs!=NULL)<<"? "<< ((lowcase_runs==NULL)?0:lowcase_runs->size()) <<"\n";
	nl_pos = NULL;
...
void Metadata::save(fstream *escritor){
	...
	if( lowcase_runs != NULL && nl_pos == NULL ){
		if( save_mode == META_SAVE_UNC ){
			...
			cerr<<"Metadata::save - Tipo desconocido "<<(unsigned long long)save_mode<<", usando META_SAVE_UNC\n";
			marca = 1;
		}
	}
	...
	cout<<"Metadata::save - marca: "<<(unsigned long long)marca<<" (save_mode: "<<(unsigned long long)save_mode<<")\n";
...
void Metadata::load(fstream *lector){
	...
	cout<<"Metadata::load - Marca: "<<(unsigned long long)marca<<"\n";
	if( marca == 0 ){
...
//		loadNewLinesVarByte(lector);
	...
	cout<<"Metadata::load - Fin (save_mode: "<<(unsigned long long)save_mode<<")\n";
}
...
unsigned long long Metadata::size(){
...
//	cout<<"Metadata::size - size: "<<size<<" (save_mode: "<<(unsigned long long)save_mode<<")\n";
	return size;
}

unsigned long long Metadata::sizeLowcase(){
	...
	if( save_mode == META_SAVE_UNC ){
		...
		for(unsigned long long i = 0; i < lowcase_runs->size(); ++i){
...
//				break;
//			}
//			size += utils.size_varbyte((unsigned long long)ini);
//			size += utils.size_varbyte((unsigned long long)fin);
		...
		cerr<<"Metadata::sizeLowcase - Tipo desconocido "<<(unsigned long long)save_mode<<"\n";
	}
	return size;
}

unsigned long long Metadata::sizeNewLines(){
	unsigned long long size = 0;
	if( save_mode == META_SAVE_UNC ){
...
//		cout<<"Metadata::sizeNewLines - unc, size: "<<size<<" ("<<nl_pos->size()<<" pos)\n";
		...
		unsigned long long last = 0;
		for(unsigned long long i = 0; i < nl_pos->size(); ++i){
...
//		cout<<"Metadata::sizeNewLines - Calculando tamaño de vbyte EX\n";
		...
		unsigned long long size_nl = nl_pos->size();
		map<unsigned long long, unsigned long long> mapa_deltas;
		map<unsigned long long, unsigned long long>::iterator it_deltas;
		unsigned long long pos;
		unsigned long long last = 0;
		for(unsigned long long i = 0; i < size_nl; ++i){
			pos = nl_pos->at(i) - last;
			last = nl_pos->at(i);
			if( pos > max_int ){
			...
			mapa_deltas[(unsigned long long)pos]++;
		}
		unsigned long long nl_norm = 0;
		unsigned long long mejor_conteo = 0;
...
//		cout<<"Metadata::sizeNewLines - nl_norm: "<<nl_norm<<", preparando excepciones\n";
		...
		vector< pair<unsigned long long, unsigned long long> > nl_ex;
		last = 0;
		for(unsigned long long i = 0; i < size_nl; ++i){
			pos = nl_pos->at(i) - last;
			last = nl_pos->at(i);
			...
			if(pos != nl_norm){
				nl_ex.push_back( pair<unsigned long long, unsigned long long>(i, (unsigned long long)pos) );
			}
		}
...
//		cout<<"Metadata::sizeNewLines - Preparando guardado de excepciones\n";
		unsigned long long size_ex = nl_ex.size();
		BitsUtils utils;
//		cout<<"Metadata::sizeNewLines - guardando "<<size_ex<<" excepciones\n";
		...
		last = 0;
		for(unsigned long long i = 0; i < size_ex; ++i){
...
//				break;
//			}
//			size += utils.size_varbyte((unsigned long long)pos);
...
//		cout<<"Metadata::sizeNewLines - total bytes: "<<size<<"\n";
		...
		cerr<<"Metadata::sizeNewLines - Tipo desconocido "<<(unsigned long long)save_mode<<"\n";
	}
	return size;
}

void Metadata::adjustCase(char *buff, unsigned long long ini, unsigned long long length){
...
//	}
	...
	unsigned long long h = lowcase_runs->size() - 1;
...
//		cout<<"Metadata::adjustCase - Ajustando ["<<par_ini<<", "<<par_fin<<"]\n";
		for(unsigned long long i = (unsigned long long)par_ini; i <= (unsigned long long)par_fin; ++i){
...
unsigned long long Metadata::totalNewLines(){
	if(nl_pos == NULL ){
		return 0;
	}
	return nl_pos->size();
...
unsigned long long Metadata::countNewLines(unsigned long long pos){
...
//	cout<<"Metadata::countNewLines - Inicio (pos: "<<pos<<")\n";
	...
	unsigned long long l = 0;
	unsigned long long h = nl_pos->size() - 1;
	unsigned long long m;
	while(l < h){
		m = l + ((h-l)>>1);
		if( nl_pos->at(m) < pos ){
...
//	cout<<"Metadata::countNewLines - Fin (total: "<<cur_pos<<")\n";
	...
	*/
	...
	cout<<"Metadata::countNewLines - Inicio (pos: "<<pos<<")\n";
	unsigned long long cur_pos = 0;
	unsigned long long cur_nl = 0;
	unsigned long long cur_ex = 0;
	//Solo usar skip list si esta definida, omitirla de otro modo
...
void Metadata::adjustNewLines(char *buff, unsigned long long ini, unsigned long long length, unsigned long long nl_izq, unsigned long long nl_med, char *copy_buff){
...
//	cout<<"Metadata::adjustNewLines - Inicio (ini: "<<ini<<", length: "<<length<<", nl_izq: "<<nl_izq<<", nl_med: "<<nl_med<<")\n";
	...
	unsigned long long buff_size = length + nl_med;
	...
	unsigned long long cur_write = 0;
	for(unsigned long long i = nl_izq; i < nl_med + nl_izq; ++i){
		pos = nl_pos->at(i);
//		cout<<"Metadata::adjustNewLines - pos: "<<pos<<", ini: "<<ini<<", cur_read: "<<cur_read<<", cur_write: "<<cur_write<<"\n";
		...
		unsigned long long len_cpy = (unsigned long long)(pos - last_pos - 1);
		last_pos = pos;
...
//		cout<<"Metadata::adjustNewLines - Copia final (cur_write: "<<cur_write<<", buff_size: "<<buff_size<<", cur_read: "<<cur_read<<", last_pos: "<<last_pos<<")\n";
		...
		unsigned long long len_cpy = buff_size - cur_write;
...
void Metadata::filterNewText(const char *in_buff, unsigned long long original_length, unsigned long long original_pos_ini, char *out_buff, unsigned long long &adjusted_length, unsigned long long &adjusted_pos_ini){
//void Metadata::filterNewText(const char *in_buff, unsigned long long original_length, unsigned long long original_pos_ini, char *out_buff, unsigned long long &adjusted_length, unsigned long long &adjusted_pos_ini){
	...
	unsigned long long r_ini = countNewLines(original_pos_ini);
	unsigned long long r_fin = countNewLines(original_pos_ini + original_length);
...
//	prepareNewLinesSkip();
	...
	for(unsigned long long i = 0; i < original_length; ++i){
		if( in_buff[i] == '\n' ){
...
//	cout<<"Metadata::saveLowcaseUncompress - Inicio\n";
	...
	unsigned long long size = lowcase_runs->size();
...
void Metadata::saveLowcaseVarByte(fstream *escritor){
	...
	unsigned long long worst_case = 5 * 2 * size;
...
//			break;
//		}
//		cur_byte += utils.write_varbyte(buff + cur_byte, (unsigned long long)ini);
//		cur_byte += utils.write_varbyte(buff + cur_byte, (unsigned long long)fin);
...
//	cout<<"Metadata::saveLowcaseVarByte - Guardando "<<cur_byte<<" bytes ("<<((unsigned long long*)buff)[0]<<")\n";
...
//	cout<<"Metadata::loadLowcaseVarByte - Cargando "<<n_bytes<<" bytes\n";
	...
	BitsUtils utils;
	unsigned long long ini, fin;
//	unsigned long long delta_ini, delta_fin;
	unsigned long long sum = 0;
//	cout<<"Metadata::loadLowcaseVarByte - Preparando "<<size<<" pares ("<<((unsigned long long*)buff)[0]<<")\n";
	for(unsigned long long i = 0; i < size; ++i){
...
//	cout<<"Metadata::saveNewLinesUncompress - Inicio\n";
	...
	unsigned long long size = nl_pos->size();
	escritor->write((char*)&size, sizeof(int));
	//Version Directa
	unsigned long long pos;
...
//	cout<<"Metadata::saveNewLinesVarByte - Inicio\n";
	...
	unsigned long long worst_case = 5 * size;
	unsigned char *buff = new unsigned char[ worst_case ];
	unsigned long long cur_byte = 0;
	BitsUtils utils;
	unsigned long long pos;
...
//			break;
//		}
//		cur_byte += utils.write_varbyte(buff + cur_byte, (unsigned long long)pos);
		
		cur_byte += utils.write_varbyte(buff + cur_byte, pos);
//		cout<<"["<<nl_pos->at(i)<<"] -> ["<<pos<<"]\n";
	}
//	cout<<"Metadata::saveNewLinesVarByte - Guardando "<<cur_byte<<" bytes ("<<((unsigned long long*)buff)[0]<<")\n";
	escritor->write((char*)&cur_byte, sizeof(int));
	escritor->write((char*)buff, cur_byte);
	delete [] buff;
...
//	cout<<"Metadata::loadNewLinesVarByte - Cargando "<<n_bytes<<" bytes\n";
	...
	unsigned long long cur_byte = 0;
	BitsUtils utils;
	unsigned long long delta_pos;
	unsigned long long pos;
	unsigned long long sum = 0;
//	cout<<"Metadata::loadNewLinesVarByte - Preparando "<<size<<" pares ("<<((unsigned long long*)buff)[0]<<")\n";
	for(unsigned long long i = 0; i < size; ++i){
...
//	cout<<"Metadata::saveNewLinesVarByteEx - Preparando guardado de excepciones\n";
	unsigned long long size_ex = nl_ex.size();
	unsigned long long worst_case = 5 * size_ex;
	unsigned char *buff = new unsigned char[ worst_case ];
	unsigned long long cur_byte = 0;
	BitsUtils utils;
...
//	cout<<"Metadata::saveNewLinesVarByteEx - Guardando "<<cur_byte<<" bytes ("<<((unsigned long long*)buff)[0]<<")\n";
	escritor->write((char*)&cur_byte, sizeof(int));
	escritor->write((char*)buff, cur_byte);
	delete [] buff;
...
void Metadata::loadNewLinesVarByteEx(fstream *lector){
	...
	//Datos
	unsigned long long size_nl = 0;
	unsigned long long size_ex = 0;
	unsigned long long nl_norm = 0;
...
//	cout<<"Metadata::loadNewLinesVarByteEx - Cargando "<<n_bytes<<" bytes\n";
	...
	unsigned long long cur_byte = 0;
	BitsUtils utils;
	unsigned long long pos_ex;
	unsigned long long val_ex;
	unsigned long long sum = 0;
	vector< pair<unsigned long long, unsigned long long> > nl_ex;
//	cout<<"Metadata::loadNewLinesVarByteEx - Preparando "<<size_ex<<" exs\n";
	for(unsigned long long i = 0; i < size_ex; ++i){
		...
		nl_ex.push_back( pair<unsigned long long, unsigned long long>(pos_ex, val_ex) );
...
//		cout<<"["<<nl_pos->at(0)<<"]\n";
//	}
	for(unsigned long long i = 1; i < size_nl; ++i){
