...
*/
	...
	for( unsigned long long i = 0; i < n_factors; ++i ){
		...
		if (cur_f != n_factors-1) arr_x_new[cur_f] = fm_index.isa[tu+lu];
	}
...
pair<unsigned long long, unsigned long long> RelzIndex::getRangeX(const char *pattern){
	...
	if (level < LEVEL_BINARY_SEARCH) fm = arr_x[izq]; 	
        ...
        fm = arr_y[wt[izq]]; // We use WT + Y to reproduce X (which is not stored)
	if( factorLess<FactorsIteratorReverse>(fm-1, pattern, pat_len) ){
		++izq;
	}
	...
	if (level < LEVEL_BINARY_SEARCH) fm = arr_x[der];
            ...
            fm = arr_y[wt[der]]; // We use WT + Y to reproduce X (which is not stored)
