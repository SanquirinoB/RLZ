...
RelzIndexHash::RelzIndexHash(vector<pair<unsigned long long, unsigned long long> > &factors, char *full_text, unsigned long long _len_text, const char *_ref_text, unsigned long long _len_ref, KarpRabin *_karp_rabin){
...
//		cout << "(" << factor.first << ", " << factor.second << ", " << cur_pos << ") - cur_start: " << cur_start << "\n";
	...
	timer.reset();
//	cout << "Factors Sorted: \n";
//	for( unsigned long long i = 0; i < factors_sort.size(); ++i ){
...
//	cout << "-----\n";
	...
	cout << "RelzIndexHash - Preparing KarpRabin Structures\n";
	...
	unsigned long long log_n = bits_utils.n_bits(ref_text->length());
	...
	unsigned long long processed_text = log_n;
	while( processed_text < ref_text->length() ){
		unsigned long long word_len = log_n;
...
//		cout << "Agregando " << kr_total << " / " << kr_test << " (kr1: " << kr1 << ", " << kr2 << ", word_len: " << word_len << ")\n";
	...
	for(unsigned long long i = 1; i < factors_start.size(); ++i){
		unsigned long long word_len = factors_start[i] - factors_start[i-1];
...
//		cout << "RelzIndexHash - Adding hash de \"" << s << "\" " << kr_total << " / " << karp_rabin->hash(s) << "\n";
	...
	unsigned long long word_len = len_text - factors_start.back();
	unsigned long long kr1 = arr_kr_s.back();
	...
	// Obviamente esto es solo para construccion y los datos usados no se almacenan, solo los datos de los nodos
...
void RelzIndexHash::findTimes(const string &pattern, vector<unsigned long long> &results, bool use_hash){
...
//		pair<unsigned long long, unsigned long long> r1 = tree_x.getRange(kr_pat_rev_vector, i, pattern_rev);
		pair<unsigned long long, unsigned long long> r1 = tree_x.getRangeRev(kr_pat_rev_vector, i, pattern_rev, use_hash);
...
//		cout << "-----  tree_y.getRange -----\n";
		pair<unsigned long long, unsigned long long> r2 = tree_y.getRange(kr_pat_vector, i, pattern, use_hash);
...
//			cout << "text_x: ";
			for(unsigned long long pos = 0; pos < i; ++pos){
				char c = it_x.next();
...
//				cout << "text_y: ";
				for(unsigned long long pos = 0; pos < pattern.length()-i; ++pos){
					char c = it_y.next();
...
void RelzIndexHash::recursive_rmq(unsigned long long ini, unsigned long long fin, unsigned long long min_pos, unsigned long long occ_ref, vector<unsigned long long> &results){
